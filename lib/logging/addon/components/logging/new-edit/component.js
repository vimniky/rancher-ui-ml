  ...
  actions: {
    save(cb) {
      ...
      model.setProperties({
        ...
        [`${targetType}Config`]: get(model, `${targetType}.config`),
        limitsCPU: get(model, 'limitsCPU'),
        limitsMemory: get(model, 'limitsMemory'),
        requestsCPU: get(model, 'requestsCPU'),
        requestsMemory: get(model, 'requestsMemory'),
      });
